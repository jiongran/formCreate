<template>
  <el-form label-position="top" v-if="data" :key="data.key">
    ...
    <el-form-item label="默认内容" v-if="hasKey('defaultValue') &&
    (data.type === 'input' ||
      data.type === 'password' ||
      data.type === 'textarea' ||
      data.type === 'text' ||
    ...
    <el-form-item label="布局方式" v-if="hasKey('inline')">
      <el-radio-group v-model="data.options.inline">
        <el-radio-button label="行内" :value="true" >行内</el-radio-button>
        <el-radio-button label="块级" :value="false" >块级</el-radio-button>
      </el-radio-group>
    </el-form-item>
    ...
    <el-form-item :label="(data.type === 'echarts-line'||data.type === 'echarts-bar')?'静态数据':'选项'" v-if="hasKey('options')">
      <template v-if="data.type === 'echarts-line'||data.type === 'echarts-bar'">
        <el-radio-group v-model="data.options.defaultValue" style="margin-top: 8px">
              <Draggable tag="ul" item-key="index" ghostClass="ghost" handle=".drag-item" :group="{ name: 'options' }"
                :list="data.options.options">
                  <template v-for="(element, index) in data.options.options">
                    <div style="display: flex; align-items: center; margin-bottom: 5px">
                      <el-radio :label="element.label" :value="element.value" style="margin-right: 0px; margin-bottom: 0">
        ...
        width: data.options.showLabel ? '90px' : '180px'
                      ...
                      <SvgIcon style="margin: 0 5px; cursor: move" name="item" className="drag-item" />
            ...
            <el-button type="text" @click="handleInsertEchartsOption">添加选项</el-button>
          </div>
      </template>
      <template v-else>
          <el-radio-group v-model="data.options.remote">
          <el-radio-button label="静态数据" :value="false">静态数据</el-radio-button>
          <el-radio-button label="远端数据" :value="true">远端数据</el-radio-button>
        </el-radio-group>
              ...
              <Draggable tag="ul" item-key="index" ghostClass="ghost" handle=".drag-item" :group="{ name: 'options' }"
                :list="data.options.options">
                  ...
                  <li style="display: flex; align-items: center; margin-bottom: 5px">
                    <el-checkbox :label="element.label" style="margin-right: 0px; margin-bottom: 0">
    ...
    <template v-if="data.type === 'img-upload'">
      <el-form-item label="模式">
        <el-radio-group v-model="data.options.listType">
          ...
          <el-radio-button label="picture" value="text">picture</el-radio-button>
          <el-radio-button label="picture-card" value="text">picture-card</el-radio-button>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="上传请求方法">
        <el-radio-group v-model="data.options.method">
          <el-radio-button label="post" value="post">POST</el-radio-button>
          <el-radio-button label="put" value="put">PUT</el-radio-button>
          <el-radio-button label="get" value="get">GET</el-radio-button>
          <el-radio-button label="delete" value="delete">DELETE</el-radio-button>
        </el-radio-group>
      </el-form-item>
    </template>
    ...
    <template v-if="data.type === 'grid'">
      ...
      <el-form-item label="列配置项">
        ...
        <div>
          <el-button link @click="handleInsertColumn">
            添加列
          </el-button>
        </div>
      </el-form-item>
      ...
      <el-form-item label="垂直对齐方式">
        <el-radio-group v-model="data.options.align">
          <el-radio-button label="顶部对齐" value="居中对齐">顶部对齐</el-radio-button>
          <el-radio-button label="居中对齐" value="居中对齐">居中对齐</el-radio-button>
          <el-radio-button label="底部对齐" value="底部对齐">底部对齐</el-radio-button>
        </el-radio-group>
      </el-form-item>
    ...
    <template v-if="data.type !== 'grid'">
      <el-form-item label="操作属性" v-if="hasKey('rules') ||
    hasKey('readonly') ||
    hasKey('disabled') ||
    hasKey('allowClear')
    ">
      ...
      <template v-if="hasKey('rules')">
        ...
        <el-form-item label="触发时机">
          <el-radio-group v-model="data.options.rules.trigger">
            <el-radio-button label="blur" value="blur">Blur</el-radio-button>
            <el-radio-button label="change" value="change">Change</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="校验类型">
          <el-select v-model="data.options.rules.type">
            <el-option value="string" label="字符串">字符串</el-option>
            <el-option value="number" label="数字">数字</el-option>
            <el-option value="boolean" label="布尔值">布尔值</el-option>
            <el-option value="method" label="方法">方法</el-option>
            <el-option value="regexp" label="正则表达式">正则表达式</el-option>
            <el-option value="integer" label="整数">整数</el-option>
            <el-option value="float" label="浮点数">浮点数</el-option>
            <el-option value="array" label="数组">数组</el-option>
            <el-option value="object" label="对象">对象</el-option>
            <el-option value="enum" label="枚举">枚举</el-option>
            <el-option value="date" label="日期">日期</el-option>
            <el-option value="url" label="URL地址">URL地址</el-option>
            <el-option value="hex" label="十六进制">十六进制</el-option>
            <el-option value="email" label="邮箱地址">邮箱地址</el-option>
            <el-option value="any" label="任意类型">任意类型</el-option>
          </el-select>
        </el-form-item>
      </template>
    </template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const handleInsertEchartsOption =  () => {
      const index = data.value.options.options.length + 1
      data.value.options.options.push({
        label: `Option ${index}`,
        value: `${index * 10}`
